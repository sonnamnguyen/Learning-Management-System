    ...
    public List<AssessmentType> importExcel(MultipartFile file) {
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream())) {
            ...
            for (int i = 1; i < rowCount; i++) { // Start from 1 to skip the header row
                ...
                if (row != null) {
                    String assessmentTypeName = row.getCell(0).getStringCellValue(); // Assume assessmentType name is in column 0
                    Cell cell = row.getCell(0); // Assume assessmentType name is in column 0
